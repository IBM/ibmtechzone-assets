...
def response(df, file_name):
    ...
    def map_response(issue):
        ...
        if pd.isna(issue):
            return "Please help us understand this issue you are facing in depth by writing it to company-name.co.in. We will look into this with priority. Thank you, company-name team"
    ...
    for index, row in df.iterrows():
        ...
        if row['Response'] == 'Please help us understand this issue you are facing in depth by writing it to company-name.co.in. We will look into this with priority. Thank you, company-name team':
            
            if row['Star Rating'] >= 3:
                df.at[index, 'Response'] = 'Thanks for the feedback, Happy banking with company-name'
             
            elif row['Star Rating'] == 3:
                df.at[index, 'Response'] = 'Thanks for the neutral feedback, please let us know in case of any issues. Thanks, company-name'

            else:
                df.at[index, 'Response'] = 'Sorry to hear, Please help us understand this issue you are facing in depth by writing it to company-name.co.in. We will look into this with priority. Thank you, company-name team.'
