...
def process_xml_file(xml_file_path):
    ...
    if st.session_state.submit_clicked:
        ...
        if not st.session_state.generate_sql_clicked:
            if st.button("Generate SQL Query"):
                ...
                # response = get_sql_query(matching_texts, os.getenv("ENDPOINT_URL1"), iam_access_token)
                # response1 = get_inst_for_query_gen(matching_texts, inst1, os.getenv("ENDPOINT_URL1"), iam_access_token)
    ...
    Mention the appropriate transformation on columns using information available in destination. \n'''+" " + matching_texts)
                ...
                # st.write(response1)
                response = get_sql_query_for_inst(instructions, inst2, os.getenv("ENDPOINT_URL2"), iam_access_token)
