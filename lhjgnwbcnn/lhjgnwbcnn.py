...
with gr.Blocks(theme=gr.themes.Soft(), css="footer {visibility: hidden;}") as demo:
    gr.Image("banner1.jpg", show_label=False)
    gr.Markdown("<center><span style='font-size: 26px; font-weight: bold;'>Sentiment Analysis</span></center>")
    
    with gr.Row():
        with gr.Column():
            input_textbox = gr.Textbox(label="Customer conversation summary", lines=2, placeholder="Paste conversation summary here")
        with gr.Column():
            output_textbox = gr.Textbox(label="Sentiment analysis results", lines=2)
