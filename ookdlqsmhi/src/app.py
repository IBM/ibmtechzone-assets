...
import re
import io
from utils import data_preprocess, response_generation, response_generation_llm, data_preprocess,department_mapping, good_modification
from BOI_issue_prediction import boi_prediction
from utils import extracted_keywords
from sklearn.preprocessing import MinMaxScaler
#from utils import issue_prediction_list
...
file_name = "dummy_Issue_responses.xlsx"
device_grouping = "Device_Grouping.xlsx"
...
# # function for the device analysis
def device_analysis(test_data):
	device_grouping_df = pd.read_excel(device_grouping)
	test_data['Review Date'] = pd.to_datetime(test_data['Review Last Update Date and Time'], format='%Y-%m-%d', errors='coerce').dt.date
	test_data['Week'] = pd.to_datetime(test_data['Review Last Update Date and Time'], format='%Y-%m-%d', errors='coerce').dt.isocalendar().week
	df_final = pd.merge(test_data, device_grouping_df, on = "Device", how = "left")
	df_final['Device Manufacturer'] = df_final['Device Manufacturer'].fillna('')

	for i in range(len(df_final)):
		if df_final.loc[i,'Device Manufacturer']=='':
			ele = df_final.loc[i,'Device']
			if isinstance(ele, str):
				if len(ele)>0:
					df_final.loc[i,'Device Manufacturer'] = 'Others'
