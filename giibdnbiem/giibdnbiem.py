...
async def get_pdf_text(pdf_url):
    text = ""
    ...
    try:
        ...
        while response_info is None and attempt < max_attempts:
            try:
                ...
                if response_info is None:
                    ...
                    print(f"Status: {response_info.status}")
        ...
        if response_info and response_info.status == 200:
            if clean_link.lower().endswith('.pdf'):
                # Process PDF file
                pdf_text = await get_pdf_text(clean_link)
                page_data = {
                    'url': clean_link,
                    'title': None,
                    'text': pdf_text,
...
if __name__ == "__main__":
    ...
    * file_ext is the extension in which you want the file to be saved in. For example, json.
    ...
    urls = [
    ("https://www.example.com/route1/","https://www.example.com/",["/route1"],"name_of_output_file","json")
